...
class MethodDispatchTest(IronPythonTestCase):
    ...
    def test_missing_value(self):
        ...
        enum_int_zero_names = ("(EnumInt)Zero", "(EnumInt)MinUShort")
        self.assertIn(tst.Test_53(), enum_int_zero_names)
        self.assertIn(tst.Test_54(), enum_int_zero_names)
        if not is_mono:
            self.assertIn(tst.Test_55(), ("(EnumUInt)Zero", "(EnumUInt)MinUInt"))
            self.assertIn(tst.Test_56(), ("(EnumUInt)Zero", "(EnumUInt)MinUInt"))
            self.assertIn(tst.Test_57(), ("(EnumLong)Zero", "(EnumLong)MinUInt"))
            self.assertIn(tst.Test_58(), ("(EnumLong)Zero", "(EnumLong)MinUInt"))
            self.assertIn(tst.Test_59(), ("(EnumULong)Zero", "(EnumULong)MinUInt"))
            self.assertIn(tst.Test_60(), ("(EnumULong)Zero", "(EnumULong)MinUInt"))
